...
Reusable metamodel HTTP lives under `/api/v1/metamodel`, not under `/api/v1/assets/{id}/…`. That keeps the feature in one prefix, leaves Marmot’s collection-first asset routes untouched, and avoids Go 1.22+ ServeMux conflicts (`/assets/{id}/<literal>` overlaps `/assets/<literal>/{id}`).

| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/v1/metamodel` | Effective composed schema |
| `PATCH` | `/api/v1/metamodel/assets/{id}` | Partial governed fields (`If-Match`) |

Do not add `/api/v1/assets/{id}/fields` or other nested `{id}` literals. New metamodel operations join this prefix. Native Marmot mutations stay as they are (`/assets/tags/{id}`, `/assets/{id}` PUT, …).
